...
const sanitizeTestName = (name: string) =>
    name
...
const collectLogFiles = () => {
    ...
    return entries
        .filter(
            (entry) =>
                entry.isFile() && LOG_PREFIXES.some((prefix) => entry.name.startsWith(prefix)),
        )
        .map((entry) => path.join(LOG_DIR, entry.name));
};

export const archiveFailedTestLogs = (testName: string) => {
    ...
    try {
        ...
        const result = spawnSync('tar', ['-czf', archivePath, '-C', tempDir, '.'], {
            stdio: 'inherit',
        });
        if (result.status !== 0) {
